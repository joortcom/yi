...
_quote, _if, _set, _define, _lambda, _begin, _definemacro, _macroexpand = list(map(Sym, 
"quote   if   set!  define   lambda   begin   define-macro  macroexpand".split()))
...
def atom(token):
    ...
    elif token[0] == '"': return token[1:-1].encode('utf-8').decode('unicode-escape')
...
def to_string(x):
    ...
    elif isa(x, str): return '"%s"' % x.encode('unicode-escape').decode("utf8").replace('"',r'\"')
...
def eval(x, env=global_env):
    ...
    while True:
        if isa(x, Symbol):       # variable reference
            ...
            for exp in x[1:-1]:
                eval(exp, env)
            x = x[-1]            # NOTE: x will be eval-ed (and return) in the while loop's next itr
        else:                    # (proc exp*)
            exps = [eval(exp, env) for exp in x]
            proc = exps.pop(0)
            if isa(proc, Procedure):
                x = proc.exp
                env = Env(proc.parms, exps, proc.env)  # NOTE: will continue the loop to eval with proc.env as outer env!
            else:
                return proc(*exps)
